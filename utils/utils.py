...
class CTFIDFVectorizer(TfidfTransformer):
    ...
    def __TFIDF_on_group(self, df, groupby, text_column_name, stopword_lang):
        text_per_class = df.groupby(groupby, as_index=False).agg(
            {text_column_name: ' '.join}).reset_index().set_index(groupby)
        nltk.download('stopwords')
        count_vectorizer = CountVectorizer(stop_words=stopwords.words(
            stopword_lang)).fit(text_per_class[text_column_name])
    ...
    def get_most_prominent_words(self, dc, groupby, text_column_name, nr_of_ranks, stopword_lang):
        ...
        ctfidf, count, words, doc_names = self.__TFIDF_on_group(
            df, groupby, text_column_name, stopword_lang)

        words_per_class = {doc_name: list([words[index] for index in ctfidf[i].argsort(
        )[-nr_of_ranks:]]) for i, doc_name in enumerate(doc_names)}

        words_tf_per_class = {doc_name: list([words[index] for index in count[i].argsort(
        )[-nr_of_ranks:]]) for i, doc_name in enumerate(doc_names)}
    ...
    def get_similarity_matrix(self, dc, groupby, text_column_name, stopword_lang):
        ...
        ctfidf, _, _, doc_names = self.__TFIDF_on_group(
            df, groupby, text_column_name, stopword_lang)
...
class Translator(object):
    ...
    def translate_text(self, text):
        ...
        try:
            res = requests.post(
                f"https://api-free.deepl.com/v2/translate", params=params)
...
class MatchCounter(object):
    ...
    def nestify(self, dc, col_order: list, inner_cols: list) -> dict:
        ...
        df = pd.DataFrame.from_dict(dc)
        assert not df[col_order + inner_cols].isnull().values.any(
        ), "No undefined values allowed to instatiate structure"
        col = col_order.pop(0)
        f = self.__nestify_rec(copy.deepcopy(col_order), inner_cols)
    ...
    def _process_leafs(self, ds, text: str):
        def f(t, x): return x.update({"agg": {"sum": sum([len(re.findall(re.compile(term) if c else re.compile(
            term, re.IGNORECASE), t)) for term, c in zip(x["regex"], x["case-sensitive"])])}})
        partial = functools.partial(f, text)
        def lc(x): return True if any([k in ["regex"]
                                       for k in x.keys()]) else False
        self._iter_leaf_item(ds, partial, lc)

    def _process_inner_nodes(self, ds):
        def agg(x): return x.update(
            {"agg": {"sum": sum([x[y]["agg"]["sum"] for y in x.keys()])}})
        def lc(x): return True if any([k in ["regex"]
                                       for k in x.keys()]) else False
    ...
    def __rec_flatten_by(self, ds, on, to_fill, leaf_criterion, depth_control, keystack=[], depth=0):
        for key in list(set(ds.keys()) - set(["agg"])):

            if key in depth_control.keys() and depth_control[key] > depth:
                assert False, "Structural incoherence, cannot flatten on a inner node, where a non-descendant of that name exists."
            elif key in depth_control.keys() and depth_control[key] < depth:
                assert key in keystack if keystack != [
                ] else True, "Found a node with the same name as a non-ancestor"
                warnings.warn(
                    "Skipping key of descendant of same name in a lower hierarchy level.")
                # print(keystack)
                continue

            to_fill.update({key: ds[key]["agg"][on] if key not in to_fill.keys(
            ) else ds[key]["agg"][on] + to_fill[key]})
            depth_control.update({key: depth})
            ...
            if leaf_criterion(ds[key]):
                ...
                # print(ds[key]["agg"][on])
                self.__rec_flatten_by(
                    ds[key], "sum", to_fill, leaf_criterion, depth_control, [] + [key], depth+1)
    ...
    def flatten_by(self, ds, on):
        ...
        def lc(x): return True if any([k in ["regex"]
                                       for k in x.keys()]) else False
        self.__rec_flatten_by(ds, on, to_fill, lc, dc)
        return to_fill
