...
class Learner(object):
    ...
    def _idx_columnwise_nearest(self, matrix, nearestto, n_nearest):
        ...
        for mcol in matrix.T:
            idx = (np.abs(mcol - nearestto)
                   ).argpartition(n_nearest)[:n_nearest]
    ...
    def get_predicts(self):
        ...
        predicts = np.array(self.learners[0].predict(
            self.X["unlabelled"])).tolist()
        # TODO: Resolve
        probas = np.array(self.learners[0].predict_proba(
            self.X["unlabelled"])).tolist()
