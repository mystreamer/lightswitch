...
class CTFIDFVectorizerTest(unittest.TestCase):
    def setUp(self):
        self.text = ["This is a test on this system",
                     "This is not a test just random", "Here is another test sentence"]
        self.count_matrix = CountVectorizer().fit_transform(self.text)

    def test_csr_output(self):
        ctfidf = CTFIDFVectorizer().fit_transform(
            self.count_matrix, n_samples=len(self.text)).toarray()
...
class MatchCounterTest(unittest.TestCase):
    def setUp(self):
        ...
        self.flattened = {"consonants": 14, "first_two_c": 7,
                          "second_two_c": 7, "vowels": 5, "first_two_v": 2, "second_two_v": 3}

    def test_nestify(self):
        ret = self.mc.nestify(self.listed, col_order=[
                              "letter_cat", "letter_order"], inner_cols=["regex", "case-sensitive"])
        assert ret == self.nested

    def test_nestify_w_nones(self):
        self.assertRaises(AssertionError, self.mc.nestify, self.listed_w_none, col_order=[
                          "letter_cat", "letter_order"], inner_cols=["regex", "case-sensitive"])
    ...
    def test_incoherent(self):
        ...
        ret = self.mc.nestify(self.listed, col_order=[
                              "letter_order", "letter_cat"], inner_cols=["regex", "case-sensitive"])
    ...
    def test_depth_control_warning(self):
        inp = {"A": {"A": {"regex": 1, "agg": {"sum": 5}}, "agg": {"sum": 5}}, "B": {"A": {"regex": 1, "agg": {
            "sum": 3}}, "agg": {"sum": {3}}}, "C": {"X": {"regex": 1, "agg": {"sum": 5}}, "agg": {"sum": 5}}}
