...
class ActiveLearningTestMultiClass(unittest.TestCase):
	def setUp(self):
		...
		self.learner_ml = Learner("testLearnerML", n_suggest=1, X=self.X, y=self.y_ml, multilabel=True)
	...
	def test_getting_n_nearest_per_col(self):
		...
		res = self.learner._idx_columnwise_nearest(matrix, nearestto=5, n_nearest=1)
		print(res)
		assert res == [[1], [1], [2]]
