...
def preprocess(foo_list, language_list, pos_tags_to_keep, filter_list, nr_workers = cpu_count()):
    ...
    chunks = [foo_list[x:x+chunk_size] for x in range(0, len(foo_list), chunk_size)]
    results = Parallel(n_jobs=nr_workers, backend="multiprocessing")(delayed(preprocess_helper)(text_list=chunk, language_list=language_list, pos_tags_to_keep=pos_tags_to_keep, filter_list=filter_list, stop_lang=stop_lang, lang_modul=lang_modul) for chunk in chunks)
...
def standard_pipeline(overview_file):
    ...
    lemmatized_list = preprocess(foo_list=text_list, language_list=lang_ids, pos_tags_to_keep=pos_tags_to_keep, filter_list=filter_list)
